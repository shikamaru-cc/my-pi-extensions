...
const GLM_MODEL_ID = "glm-4.7";

/**
 * Prefer GLM-4.7 for extraction when available, otherwise fallback to the current model.
 */
async function selectExtractionModel(
	currentModel: Model<Api>,
	modelRegistry: ModelRegistry,
): Promise<Model<Api>> {
	const glmModel = modelRegistry.find("zai", GLM_MODEL_ID);
	if (!glmModel) {
		return currentModel;
	}

	const auth = await modelRegistry.getApiKeyAndHeaders(glmModel);
	if (!auth.ok) {
		return currentModel;
	}
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			// Select the best model for extraction (prefer GLM-4.7)
